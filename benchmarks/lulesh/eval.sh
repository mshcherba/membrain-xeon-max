...
echo "[1/4] Running Baseline Unguided Intel LULESH (DDR5-only)..."
START=$(date +%s.%N)
numactl --cpunodebind=0 --membind=0 "${LULESH_BIN}" "${LULESH_ARGS[@]}" > baseline.log
END=$(date +%s.%N)
...
python3 "${MEMBRAIN_ROOT}/profiler/pebs_profiler.py" \
    ...
    "${LULESH_BIN}" "${LULESH_ARGS[@]}" >/dev/null 2>&1 || true
